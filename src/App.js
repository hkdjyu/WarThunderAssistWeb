...
function App() {
  ...
  // useEffect(() => {
  //   fetch("https://192.168.0.133:8112/state")
  ...
  const [ipAddress, setIpAddress] = useState("localhost");
    ...
    const fetchStateData = useEffect(() => {
        if (isRunning === false){
            return;
        }
        const interval = setInterval(() => {
            try{
                fetch("http://" + ipAddress + ":" + (ipPort).toString() + "/state")
                // fetch("https://" + ipAddress + ":" + (ipPort+1).toString())
                .then(response => response.json())
    ...
    const fetchIndicatorsData = useEffect(() => {
        if (isRunning === false){
            return;
        }
        const interval = setInterval(() => {
            try{
                fetch("http://" + ipAddress +":" + (ipPort).toString() + "/indicators")
                // fetch("https://" + ipAddress +":" + (ipPort+2).toString())
                .then(response => response.json())
    ...
    const fetchMapData = useEffect(() => {
        ...
        const interval = setInterval(() => {
            try{
                fetch("http://" + ipAddress +":" + (ipPort).toString() + "/map_obj.json")
                // fetch("https://" + ipAddress +":" + (ipPort+3).toString())
                .then(response => response.json())
                .then(data => {
                    setMapData(data)
                })
    ...
    const fetchMapBasicData = useEffect(() => {
        ...
        try{
            fetch("http://" + ipAddress + ":" + (ipPort).toString() + "/map_info.json")
            // fetch("https://" + ipAddress + ":" + (ipPort+4).toString())
            .then(response => response.json())
            .then(data => {
      ...
      <header className="App-header">
        <div style={{height: "90vh", display: "flex", flexDirection: "column", justifyContent: "flex-start", alignItems: "center",}}>
          ...
          <div style={{flexDirection:"column", display:"flex", justifyContent:"center", alignItems:"flex-start"}}>
              ...
              <div>
                  <input className="ipInput" type="text" style={{color:"black"}} onChange={(e) => setIpPortValue(parseInt(e.target.value)) } value={ipPortValue}></input>
                  <button className="setButton" style={{color:"black"}} onClick={() => setIpPort(parseInt(ipPortValue))}>
                      <p style={{color:"white", fontFamily:"digital-clock-font"}}>Set</p>
                  </button>       
              </div>    
          </div>
